...
## File Format
The source text file (e.g., `dsa_cards.txt`) uses a simple format:
```text
...
```bash
./venv/bin/python generate_anki_from_text.py dsa_cards.txt
```
**Output**: `dsa_cards_basic_[DATE].apkg`

### 2. `dump_apkg.py`
Extracts and visualizes the contents of an `.apkg` file. Useful for verifying deck content without opening Anki.
```bash
./venv/bin/python dump_apkg.py dsa_cards_basic_2025-12-10.apkg
```
**Output**:
...
### 3. `migrate_to_stable_ids.py`
One-time migration script. It extracts GUIDs from an existing `.apkg` and updates your text file to include them. Use this if you have an existing deck and want to switch to this text-based workflow without losing progress.
```bash
./venv/bin/python migrate_to_stable_ids.py dsa_cards.txt dsa_cards_basic_2025-12-10.apkg
```

### 5. `verify_guids.py`
Verifies that the GUIDs in a generated `.apkg` match the source text file.
```bash
./venv/bin/python verify_guids.py dsa_cards.txt dsa_cards_basic_2025-12-10.apkg
```
**Options**:
- `--verbose` or `-v`: Print the content of missing or unexpected cards to help identify discrepancies.

## Workflow
1.  **Edit**: Add or modify cards in `dsa_cards.txt`.
